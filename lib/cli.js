...
async function main() {
    ...
    const prog = new Command('vers')
        ...
        .option('--no-pre', 'do not print pre-release info')
        .option('--no-build', 'do not print build info')
        .option('-p <tag...>', 'additional pre-release tags')
        .option('-b <info...>', 'additional build infos')
        ...
        .option('--no-plus', 'print dash "-" instead of "+" before build info', false)
...
async function cmdVer(globals, options) {
    ...
    ver = await git.gitGetVersion(ver, tagPrefix, globals.gitHash, globals.p, globals.b, paths)

    const vstr = version.print(ver, globals.pre, globals.build)
    return globals.plus ? vstr : vstr.replaceAll(/\+/g, '-')
}
