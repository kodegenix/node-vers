...
async function cmdPublish(globals, options) {
    ...
    if (options.check) {
        const manifest = await npm.manifestRead()
        try {
            await npm.npmView(manifest.name, v)
            console.log(`package '${manifest.name}@${v}' already exists in repository, nothing to do.`)
            return
        }
        catch (err) {
            console.log(`package '${manifest.name}@${v}' does not exists in repository, publishing.`)
        }
    }
...
async function cmdVer(globals, options) {
    ...
    const manifest = await npm.manifestRead()
    const version = await git.gitGetVersion(manifest.version, tagPrefix, globals.preRelease, paths)
    if (globals.gitHash) {
        return version.raw
    } else {
