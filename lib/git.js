...
module.exports = {
    ...
    gitFindTags: async function (tagPrefix) {
        const res = await cmdExec(`git tag --list --format='%(refname:short):|:%(object)' '${tagPrefix}*'`)
    ...
    gitCountCommits: async function (startCommit, endCommit, dirPath) {
        const res = await cmdExec(`git log ${startCommit}..${endCommit ? endCommit : ''} --format='%H' -- ${dirPath}`)
