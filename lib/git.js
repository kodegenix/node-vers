...
module.exports = {
    ...
    gitFindCommitsForPath: async function (startCommit, endCommit, dirPath) {
    ...
    gitGetVersion: async function (tagPrefix, preReleaseTag) {
        ...
        if (tags.length) {
            const tag = tags[0]
            const commits = await this.gitFindCommitsForPath(tag.commit, head, process.cwd())
            let ver
            if (commits.length <= 1) {
                ver = `${tag.version}`
            } else {
                preReleaseTag = preReleaseTag ? preReleaseTag + '.' : ''
                ver =`${tag.version}-${preReleaseTag}${commits.length - 1}+g${commits[commits.length - 1].substr(0, 7)}`
            }
